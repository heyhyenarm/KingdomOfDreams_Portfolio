...
public class Tutorial03Main : MonoBehaviour
{
    //public PlayerMono playerMono;
    public UITutorial03Director director;
    ...
    public void Start()
    {
        //this.tutorialStage.clearGround(2);
        var tuto03_SC = SoundManager.GetSoundConnectionForThisLevel("Tutorial03");
        SoundManager.PlayConnection(tuto03_SC);
    }
    public void Init()
    {
        ...
        this.player.mono.transform.position = new Vector3(-16f, 1.4f, 3f);
        ...
        for (int i = 0; i < this.farmrows.Length; i++)
        {
    ...
    private void Update()
    {
        ...
        if (this.npc.isClear && this.building.isClear && !this.isClearStage)
        {
            ...
            var stageInfo = InfoManager.instance.StageInfos.Find(x => x.stage == 3);
            if (stageInfo == null)
            {
                this.SaveClearInfos();
            }
    ...
    {
        InfoManager.instance.AddStageInfo(3);
    ...
    private void SetArrowTarget()
    {
        ...
        {
            this.targetGo = this.tempTargetGo;

            if (this.targetGo.transform.childCount != 0)
                this.targetGo = this.targetGo.transform.GetChild(0).gameObject;
        }
    ...
    private void SubtractIngredientAmountN(NPC npc)
    {
        ...
        var fishInfo = InfoManager.instance.IngredientInfos.Find(x => x.id == 2002);
    ...
    private IEnumerator WaitForStageClearVideo(float time)
    {
        ...
        InfoManager.instance.DialogueInfo.id = 10025;
